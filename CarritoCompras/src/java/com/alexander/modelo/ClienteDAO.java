...
public class ClienteDAO {
    ...
    public Cliente Validar(String email, String pass) {
        String sql="SELECT * FROM bdcarritocompras.cliente WHERE Email=? AND Password=?";
    ...
    public int AgregarCliente(Cliente c) {
        String sql="INSERT INTO bdcarritocompras.cliente (Dni, Nombres, Direccion, Email, Password)VALUES(?,?,?,?,?)";        
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
